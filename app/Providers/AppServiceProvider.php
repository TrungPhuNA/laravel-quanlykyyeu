...
//        $this->bootDBLogger();
        try{
            $categories = Category::with('children:id,c_name,c_slug,c_parent_id,c_icon')
                ->where('c_parent_id',0)
                ->select('id','c_name','c_slug','c_avatar','c_parent_id','c_icon')
                ->get();

            View::share('categories', $categories);


            $categoriesHot = Category::where('c_hot', 1)->select('id','c_name','c_slug','c_avatar','c_icon')->get();
